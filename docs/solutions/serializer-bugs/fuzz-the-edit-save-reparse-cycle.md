...
Known limitation (documented in `createTurndownService`): a comment or suggestion anchored on pure whitespace does not survive save — turndown replaces "blank" nodes before rules run, and ProseMirror drops a whitespace-only mark span on reparse. A fix needs sentinel text through serializer, parser and editor; the fuzz ops skip whitespace-only ranges so the gap stays visible there.
